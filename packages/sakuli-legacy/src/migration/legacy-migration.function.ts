...
function writeAsyncChangeLog(changeLog: string[], node: any) {
  changeLog.push(
    `${node.loc.start.line}:${node.loc.start.column} Changed ${node.callee.name} to await ${node.callee.name}`
  );
}

function mapSahiToAsync(
  node: Node,
  changeLog: string[],
  parent: any | null
): Node {
    ...
    if (typeof node === "object")
      try {
        if ((node as any)[prop] && !Array.isArray((node as any)[prop])) {
          (node as any)[prop] = mapSahiToAsync(
            (node as any)[prop] as any,
            changeLog,
            node
          ) as any;
        }
        if ((node as any)[prop] && Array.isArray((node as any)[prop])) {
          (node as any)[prop] = ((node as any)[prop] as any).map((n: any) =>
