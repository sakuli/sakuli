...
export function createApplicationClass(
  ...
  return class SakuliApplication implements Application {
    ...
    private closeOrKillFunction(
      ...
      return runAsAction(ctx, methodName, () => {
        return new Promise<Application>(async (resolve, reject) => {
          ctx.logger.debug(
            `Trying to ${methodName} application '${this.cmd}${
              optSilent ? " silently" : ""
            }`
          );
          if (this.process) {
    ...
    public async open(): Promise<Application> {
      return runAsAction(ctx, "runCommand", () => {
        ctx.logger.debug(
          `Opening application '${this.cmd}${
            this.args.length ? ` with args ${this.args}` : ""
          }`
        );
        return new Promise<Application>((resolve, reject) => {
          try {
            if (!this.process) {
              ...
              ctx.logger.debug(
                `Application already open: PID=${this.process.pid}`
              );
            }
    ...
    public async focusWindow(windowNumber: number): Promise<Application> {
      ctx.logger.debug(
        `Focusing window #${windowNumber} of application '${this.cmd}`
      );
      throw new Error("Not Implemented");
    }
    ...
    public setSleepTime(seconds: number): Application {
      ctx.logger.debug(
        `Updating startup delay from ${this.sleepTime} to ${seconds}`
      );
      if (seconds < 0) {
        ctx.logger.warn(
          `Invalid value for sleep time: ${seconds}. Skipping update`
        );
        return this;
      }
    ...
    public async getRegion(): Promise<Region> {
      return runAsAction(ctx, "getRegion", async () => {
        ...
        return new RegionImpl(
          0,
          0,
          await ScreenApi.width(),
          await ScreenApi.height()
        );
      })();
    }
    ...
    public async getRegionForWindow(windowNumber: number): Promise<Region> {
      return runAsAction(ctx, "getRegionForWindow", async () => {
        ctx.logger.debug(
          `Determining region for window #${windowNumber} of ${this.cmd}`
        );
        return new RegionImpl(
          0,
          0,
