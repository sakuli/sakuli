  ...
  return class SakuliEnvironment implements Environment {
    ...
    public setSimilarity(similarity: number) {
      if (similarity > 0 && similarity <= 1) {
        ...
        ctx.logger.warn(
          `Invalid value for similarity: ${similarity}. Skipping update`
        );
      }
    }
    ...
    public resetSimilarity() {
      ctx.logger.debug(
        `Resetting similarity to default value ${props.sakuliEnvironmentSimilarityDefault}`
      );
    ...
    public async getRegionFromFocusedWindow(): Promise<Region> {
      return runAsAction(ctx, "getRegionFromFocusedWindow", async () => {
        const focusedWindowRegion = await ScreenApi.getRegionFromFocusedWindow();
        const RegionImpl = createRegionClass(ctx, project);
        return new RegionImpl(
          focusedWindowRegion.left,
          focusedWindowRegion.top,
          focusedWindowRegion.width,
          focusedWindowRegion.height
        );
      })();
    }
    ...
    public async runCommand(
      command: string,
      optThrowException: boolean = false
      ...
      return runAsAction(ctx, "runCommand", () => {
        ...
        return new Promise<CommandLineResult>(async (resolve, reject) => {
          try {
            ...
            ctx.logger.debug(
              `Return code: ${result.getExitCode()}, Output: ${result.getOutput()}`
            );
