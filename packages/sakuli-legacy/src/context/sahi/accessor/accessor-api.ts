...
export type DefaultAccessors = Pick<
  AccessorApi,
  Exclude<
    keyof AccessorApi,
    "_activeElement" | "_byId" | "_byText" | "_byClassName" | "_byXPath"
  >
>;
export type AccessorFunctions = Exclude<
  keyof AccessorApi,
  "_activeElement" | "_byId" | "_byText" | "_byClassName" | "_byXPath"
>;
...
export function accessorApi(): AccessorApi {
  function createAccessorFunction(...css: string[]): AccessorFunction {
    return (identifier: AccessorIdentifier, ...relations: SahiRelation[]) => {
      let extendedLocator = isAccessorIdentifierAttributesWithClassName(
        identifier
      )
        ? By.css(
            css
              .map(
                (css) => `${css}.${identifier.className.split(" ").join(".")}`
              )
              .join(", ")
          )
        : By.css(css.join(", "));
  ...
  return {
    ...
    _submit: createAccessorFunction(
