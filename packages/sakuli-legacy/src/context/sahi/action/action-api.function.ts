...
export function actionApi(
  ...
  const runAsAction = <ARGS extends any[], R>(
    name: string,
    fn: (...args: ARGS) => Promise<R>
  ): ((...args: ARGS) => Promise<R>) => {
    if (properties.skipFrames) {
  ...
  const { _blur, _focus } = focusActionApi(webDriver, accessorUtil, ctx);
  ...
  const { _setValue, _keyPress, _keyUp, _keyDown, _type } = keyboardActionApi(
    webDriver,
    accessorUtil,
    ctx
  );
