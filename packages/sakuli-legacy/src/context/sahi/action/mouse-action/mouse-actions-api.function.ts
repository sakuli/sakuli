...
export function mouseActionApi(
    ...
    if (!combo && !options) {
      ...
      if (isClickNotForced(combo, options)) {
        ...
        if (await isElementCovered(e, webDriver)) {
          throw new ElementClickInterceptedError(
  ...
  async function _check(query: SahiElementQueryOrWebElement) {
    ...
    if (
      tagName.toLocaleLowerCase() === "input" &&
  ...
  async function _setSelected(
    ...
    const valuesOrIndices =
      typeof optionToSelect === "string" || typeof optionToSelect === "number"
        ? [optionToSelect]
        : optionToSelect;
    const isNumberArray = (arr: (number | string)[]): arr is number[] =>
  ...
  function validateComboAndOptions(
    combo: undefined | string | ClickOptions,
    options: undefined | ClickOptions
  ): void {
    if (combo && isClickOptions(combo) && options && isClickOptions(options)) {
      ...
      if (combo.force !== options.force) {
        throw Error(
          `Invalid argument combination for combo ${combo} and options ${options}`
        );
      }
    }
  }

  function isClickNotForced(
    combo: undefined | string | ClickOptions,
    options: undefined | ClickOptions
  ) {
    const isComboNotForced = combo && isClickOptions(combo) && !combo.force;
    const isOptionsNotForced =
      (!combo || !isClickOptions(combo)) &&
      (!options || (options && !options.force));

    return isOptionsNotForced || isComboNotForced;
  }

  function comboClickWithActionSequence(
    combo: string,
    e: WebElement
  ): Promise<void> {
    return runActionsWithComboKeys(
