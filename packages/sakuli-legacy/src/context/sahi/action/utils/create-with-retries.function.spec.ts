...
describe("create-with-retries", () => {
  ...
  let withRetries: <ARGS extends any[], R>(
    r: number,
  ...
  beforeEach(() => {
    ...
    withRetries = createWithRetries(ctx);
  });
  ...
  it("should throw after all retries", async () => {
    ...
    await expect(retryableAction(1, 2)).rejects.toThrowError(
      /Failed on an action after 3 attempts./
    );
    expect(action).toHaveBeenCalledTimes(3);
  });

  it("should throw on error which is not StaleElementReferenceError or MoveTargetOutOfBoundsError", async () => {
    action = jest.fn().mockRejectedValueOnce(Error("Some error"));
    const retryableAction = withRetries(3, action);
