...
describe('create-with-action-context', () => {
    ...
    let withActionContext: <ARGS extends any[], R>(name: string, fn: (...args: ARGS) => Promise<R>) => ((...args: ARGS) => Promise<R>);
    ...
    describe('error in execution', () => {
        ...
        it('should invoke action and throw an error', async () => {
            ...
            await expect(wrappedActionResultPromise).rejects.toThrowError(/Error in Action/);
            expect(actionMock).toHaveBeenCalledWith(1, 'Test');
        });

        it('should start and finish an action anyway', async () => {
            await expect(wrappedAction(1, "Test")).rejects.toThrowError(/Error in Action/);
            expect(ctx.startTestAction).toHaveBeenCalledWith(expect.objectContaining({id: 'dummy'}));
            expect(ctx.updateCurrentTestAction).toHaveBeenCalledWith(expect.objectContaining({error: actionError}));
            expect(ctx.endTestAction).toHaveBeenCalled();
