...
describe("createTryAcrossFrames", () => {
  ...
  let pseudoFrames = [Symbol("frame1"), Symbol("frame2"), Symbol("frame3")];
  let contextMock: TestExecutionContext;
  let withTryAcrossFrames: <ARGS extends any[], R>(
  ...
  it("should rethrow latest error when failing in every frame", async () => {
    ...
    const actionAcrossFrames = withTryAcrossFrames(action);
    await expect(actionAcrossFrames(1, 2, 3)).rejects.toEqual(
      "Error in frame 2"
    );
  ...
  it("should throw initial error if only TimeoutErrors follow", async () => {
    ...
    action.mockRejectedValueOnce(
      new SeleniumErrors.TimeoutError("Error in frame 0")
    );
    action.mockRejectedValueOnce(
      new SeleniumErrors.TimeoutError("Error in frame 1")
    );
    action.mockRejectedValueOnce(
      new SeleniumErrors.TimeoutError("Error in frame 2")
    );
    findElementsMock.mockResolvedValueOnce(pseudoFrames);
