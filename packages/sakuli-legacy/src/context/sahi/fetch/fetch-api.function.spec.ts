  ...
  describe.each(getTestBrowserList())(
    ...
    (browser: "firefox" | "chrome", local: boolean) => {
      ...
      test('_getValue to read value "test" from  input element', async () => {
        ...
        return expect(_getValue(queryByLocator(By.css("input")))).resolves.toBe(
          "test"
        );
      });
      ...
      test.each(<["value" | undefined, string[]][]>[
        ...
        "_getOptions to return a list of options innerTexts",
        async (value: "value" | undefined, expected: string[]) => {
        ...
        `)
          ...
          return expect(_getOptions(q, value)).resolves.toEqual(
      ...
      test.each(<[string, boolean][]>[
        ...
        "_exists to resolve %s to %s",
        async (element: string, expected: boolean) => {
          const { _exists } = api;
          await driver.get(
            mockHtml(`
            <h1>Exists</h1>
      ...
      test.each(<[string, string, boolean][]>[
        ...
        '_areEqual to identify "%s" and "%s" to equal %s',
        async (
          cssSelector1: string,
          cssSelector2: string,
          expected: boolean
      ...
      test.each(<[string, boolean][]>[
        ...
        "_isVisible is %s for %s",
        async (selector: string, expected: boolean) => {
      ...
      test.each(<[boolean, string][]>[
        ...
        "_isChecked to be %s for %s",
        async (expected: boolean, selector: string) => {
      ...
      test.each(<[string, boolean][]>[
        ...
        "_containsHTML to test partial texts %s from HTML-Elements is %s",
        async (text: string, expected: boolean) => {
      ...
      test("_getSelectionText to return selected text", async () => {
        ...
        return expect(_getSelectionText()).resolves.toEqual(
      ...
      test("_fetch to resolve to an Element", async () => {
        ...
        expect(await e1.getId()).toBe(await e2.getId());
      });
    }
  );
});
