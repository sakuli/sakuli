...
export function fetchApi(
  ...
  async function _getOptions(
    ...
    return Promise.all(
      options.map((option) =>
        value === "value" ? option.getAttribute("value") : option.getText()
      )
    );
  }
  ...
  async function _areEqual(
    q1: SahiElementQueryOrWebElement,
    q2: SahiElementQueryOrWebElement
  ) {
  ...
  const { _collect, _count } = multipleElementApi(driver, accessorUtil, ctx);
