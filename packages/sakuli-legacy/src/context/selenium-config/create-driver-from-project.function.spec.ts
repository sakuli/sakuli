...
import { createDriverFromProject, Browsers } from "./create-driver-from-project.function";
...
describe('createDriverFromProject', () => {
    ...
    beforeEach(() => {
        ...
        project = mockPartial<Project>({
            objectFactory: jest.fn((type: Type<any>): any => {
                ...
                throw Error('unexpected property-class causes test to fail: ' + type.name);
            })
        })
    ...
    it.each([
        ...
        'ie',
        'edge'
    ] as Browsers[])(`should in invoke the methods according to browser`, (browser: Browsers) => {
