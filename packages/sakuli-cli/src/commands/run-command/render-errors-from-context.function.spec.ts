import { TestExecutionContext, TestCaseContext } from "@sakuli/core/dist";
...
describe('renderErrorsFromContext', () => {
    ...
    it('it should render all errors', async () => {
        ...
        expect(console.log).toHaveBeenNthCalledWith(1,expect.stringMatching(/.*case.*FailingTC.*/gm))
        expect(renderError).toHaveBeenNthCalledWith(1, error1);
        expect(console.log).toHaveBeenNthCalledWith(2,expect.stringMatching(/.*step.*FailingStep.*/gm))
