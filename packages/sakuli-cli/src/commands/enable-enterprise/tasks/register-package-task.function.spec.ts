...
describe('registerPackageTask', () => {
    ...
    const mockPackageJson = async (content: object) => {
        await fs.writeFile(join(packageJsonPath, 'package.json'), JSON.stringify(content))
    }
    ...
    it.each(<[object, string[]][]>[
    ...
    ])("should add 'package' to presetProviders %s", async (
        packageJsonContent,
        expectedPresets
    ) => {
        ...
        const packageJson = await fs.readFile(join(packageJsonPath, 'package.json'))
            .then(buf => buf.toString())
            .then(content => JSON.parse(content));
        expect(packageJson).toEqual(objectContaining({
