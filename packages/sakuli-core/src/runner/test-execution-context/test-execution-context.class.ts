...
export class TestExecutionContext extends EventEmitter implements Measurable {
  ...
  constructor(readonly logger: SimpleLogger) {
    super();
  }
  ...
  on(
    type: TestExecutionContextEventTypes | "change",
  ...
  endExecution() {
    if (this.startDate) {
      ...
      throw new Error(
        "You cannot end an execution before it has been started. Please call TestExecutionContext::startExecution before call endExecution"
      );
    }
  ...
  get testCases() {
    return this.testSuites.reduce(
      (tc, ts) => [...tc, ...ts.testCases],
      [] as TestCaseContext[]
    );
  }

  get testSteps() {
    return this.testCases.reduce(
      (ts, tc) => [...ts, ...tc.testSteps],
      [] as TestStepContext[]
    );
  }

  get testActions() {
    return this.testSteps.reduce(
      (ta, ts) => [...ta, ...ts.testActions],
      [] as TestActionContext[]
    );
  }
  ...
  get duration() {
    if (this.isExecutionFinished()) {
      ...
      throw new Error(
        "Cannot get duration until execution is marked as finished. Please call TestExecutionContext::endExecution() before accessing duration"
      );
    }
  }
  ...
  startTestSuite(testSuite: Partial<TestSuiteContext> = {}) {
    if (this.isExecutionStarted()) {
      const newTestSuite = Object.assign(new TestSuiteContext(), testSuite, {
        startDate: new Date(),
      });
  ...
  updateCurrentTestSuite(
    testSuite: Partial<TestSuiteContext>
  ): TestSuiteContext {
  ...
  private _updateCurrentTestSuite(
    testSuite: Partial<TestSuiteContext>
  ): TestSuiteContext {
    const current = throwIfAbsent(
      this.getCurrentTestSuite(),
      Error(
        "There is no current Testsuite to update. Please ensure that you already called TestExecutionContext::startTestSuite()"
      )
    );
  ...
  endTestSuite() {
    const updatedTestSuite = this._updateCurrentTestSuite({
      endDate: new Date(),
    });
  ...
  startTestCase(testCase: Partial<TestCaseContext> = {}) {
    ...
    const newTestCase = Object.assign(new TestCaseContext(), testCase, {
      startDate: new Date(),
    });
  ...
  private _updateCurrentTestCase(testCaseContext: Partial<TestCaseContext>) {
    ...
    const updatedTestCase = (suite.testCases[
      suite.testCases.length - 1
    ] = Object.assign(new TestCaseContext(), testCase, testCaseContext));
    this._updateCurrentTestSuite({
  ...
  startTestStep(testStep: Partial<TestStepContext> = {}) {
    ...
    const newTestStep = Object.assign(new TestStepContext(), testStep, {
      startDate: new Date(),
    });
    this._updateCurrentTestCase({
      testSteps: [...testcase.testSteps, newTestStep],
    });
  ...
  private _updateCurrentTestStep(testStep: Partial<TestStepContext>) {
    ...
    const updatedTestStep = (testCase.testSteps[
      testCase.testSteps.length - 1
    ] = Object.assign(new TestStepContext(), step, testStep));
    this._updateCurrentTestCase({
  ...
  startTestAction(testAction: Partial<TestActionContext>) {
    ...
    const newAction = Object.assign(new TestActionContext(), testAction, {
      startDate: new Date(),
    });
    this._updateCurrentTestStep({
      testActions: [...testStep.testActions, newAction],
    });
    this.emit(START_TESTACTION, newAction);
    this.emitChange();
  }

  updateCurrentTestAction(
    testAction: Partial<TestActionContext>
  ): TestActionContext {
   ...
   * @private
   */
  private _updateCurrentTestAction(
    testAction: Partial<TestActionContext>
  ): TestActionContext {
    const testStep = throwIfAbsent(
      this.getCurrentTestStep(),
    ...
    const updatedAction = (testStep.testActions[
      testStep.testActions.length - 1
    ] = Object.assign(new TestActionContext(), action, testAction));
    this._updateCurrentTestStep({
  ...
  endTestAction() {
    const finishedAction = this._updateCurrentTestAction({
      endDate: new Date(),
    });
  ...
  get resultState(): TestContextEntityState {
    return Math.max(
      ...this.testSuites.map((ts) => ts.state)
