...
export function Property(path: string, reader: ReadMap<any> = identity) {
  return ({ constructor }: any, property: string) => {
    ...
    Reflect.defineMetadata(
      ...
      [...propertiesInClass, { path, property, reader }],
      constructor
    );
  };
}

const onlyValue = <T>(reader: ReadMap<T>) => (v: Maybe<T>) =>
  ifPresent(v, reader, () => v);
 ...
 * @constructor
 */
export const BooleanProperty = (path: string) =>
  Property(path, BooleanPropertyReader);
 ...
 * @constructor
 */
export const NumberProperty = (path: string) =>
  Property(path, onlyValue(Number));
  ...
  Property(
    ...
    onlyValue((v) => {
      return Array.isArray(v)
        ? v.map(mapper)
        : String(v).split(delimiter).map(mapper);
    })
  );

export function getPropertyDecoratorDefinitions(
  type: Type<any>
